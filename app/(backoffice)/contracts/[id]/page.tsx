    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <h1 className="text-white text-xl font-bold">Contract #{contract.contractNo}</h1>
          <Badge variant={badgeVariant} />
        </div>
        <div className="flex gap-3">
          <button className="flex items-center gap-2 bg-slate-700 hover:bg-slate-600 text-slate-200 px-4 py-2 rounded-lg text-sm font-medium transition-colors">
            ⬇️ Download PDF
          </button>
      ...
      <div className="grid grid-cols-5 gap-5">
        ...
        <div className="col-span-2 space-y-4">
          ...
          <div className="bg-slate-800 rounded-xl border border-slate-700 p-5">
            ...
            <dl className="space-y-3">
              <div>
                <dt className="text-slate-500 text-xs">Deposit Amount</dt>
                <dd className="text-white text-2xl font-bold">${contract.depositAmount.toFixed(2)}</dd>
              </div>
              <div className="border-t border-slate-700 pt-3">
                <dt className="text-slate-500 text-xs">Daily Rate</dt>
                <dd className="text-white text-2xl font-bold">${contract.dailyRate.toFixed(2)}</dd>
              </div>
              <div className="border-t border-slate-700 pt-3">
                <dt className="text-slate-500 text-xs">Monthly Rate</dt>
                <dd className="text-white text-2xl font-bold">${contract.monthlyRate.toFixed(2)}</dd>
              </div>
              <div className="border-t border-slate-700 pt-3">
                <dt className="text-slate-500 text-xs">Battery Health Penalty</dt>
                <dd className="text-slate-300 text-sm">&gt;5% degradation incurs $200 fee</dd>
              </div>
            </dl>
          </div>
          ...
          {/* Reminders & Actions */}
          <div className="bg-slate-800 rounded-xl border border-slate-700 p-5">
            <h2 className="text-white font-semibold mb-4">Reminders & Actions</h2>
            <div className="space-y-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-slate-200 text-sm font-medium">Upcoming Expiry Notification</p>
                  <p className="text-slate-500 text-xs mt-1">Status: Last reminder sent: 2 days ago</p>
                </div>
                <span className="text-slate-400 text-lg">🕐</span>
              </div>
              <div className="flex items-center justify-between">
                <p className="text-slate-300 text-sm">Schedule Auto-Reminder</p>
                <button
                  onClick={() => setAutoReminder(!autoReminder)}
                  className={`w-10 h-6 rounded-full transition-colors relative ${autoReminder ? 'bg-teal-500' : 'bg-slate-600'}`}
                >
                  <span
        ...
        {/* PDF Viewer */}
        <div className="col-span-3 bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
          <div className="bg-slate-900 border-b border-slate-700 px-4 py-2 flex items-center justify-between text-xs text-slate-400">
            <div className="flex items-center gap-3">
              <span>☰</span>
            ...
            <div className="flex items-center gap-2">
              ...
              <button className="hover:text-slate-200">+</button>
              <span>100%</span>
              <button className="hover:text-slate-200">⬇️</button>
              <button className="hover:text-slate-200">🖨</button>
            </div>
          </div>
          <div className="flex items-center justify-center h-[500px] bg-slate-900/50 p-8">
