...
export default function ContractsPage() {
  ...
  return (
    <div className="space-y-5">
      <h1 className="text-white text-xl font-bold">Active Rental Contracts Overview</h1>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-700">
              <th className="text-left text-slate-400 text-xs font-medium px-5 py-3 uppercase tracking-wide">Contract ID</th>
              <th className="text-left text-slate-400 text-xs font-medium px-5 py-3 uppercase tracking-wide">Tenant Name</th>
              <th className="text-left text-slate-400 text-xs font-medium px-5 py-3 uppercase tracking-wide">License Plate</th>
              <th className="text-left text-slate-400 text-xs font-medium px-5 py-3 uppercase tracking-wide">Start Date</th>
              <th className="text-left text-slate-400 text-xs font-medium px-5 py-3 uppercase tracking-wide">Due Date</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map(contract => (
              <tr key={contract.id} className="border-b border-slate-700/50 hover:bg-slate-700/30 transition-colors">
                <td className="px-5 py-3">
                  <Link
                    href={`/contracts/${contract.id}`}
                    className="text-teal-400 hover:text-teal-300 text-sm font-mono transition-colors"
                  >
                    {contract.contractNo}
                  </Link>
                </td>
                <td className="px-5 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-slate-200 text-sm">{contract.customerName}</span>
                  </div>
                </td>
                <td className="px-5 py-3 text-slate-300 text-sm font-mono">{contract.vehiclePlate}</td>
                <td className="px-5 py-3 text-slate-400 text-sm">{contract.startDate}</td>
                <td className="px-5 py-3">
                  <span
