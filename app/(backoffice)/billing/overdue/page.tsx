...
export default function OverduePage() {
  return (
    <div className="space-y-5">
      <h1 className="text-white text-xl font-bold">Overdue & Debt Collection Tracker</h1>
      ...
      <div className="grid grid-cols-2 gap-4 max-w-2xl">
        <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-5">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-white text-3xl font-bold">${totalDebt.toLocaleString()}</p>
        ...
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-5">
          ...
          <p className="text-white text-3xl font-bold">{overdueInvoices.length}</p>
        </div>
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-700">
              ...
              <th className="text-left text-slate-400 text-xs font-medium px-5 py-3 uppercase tracking-wide">Total Debt</th>
              <th className="text-left text-slate-400 text-xs font-medium px-5 py-3 uppercase tracking-wide">Days Overdue</th>
              <th className="text-left text-slate-400 text-xs font-medium px-5 py-3 uppercase tracking-wide">Last Contacted</th>
              <th className="text-left text-slate-400 text-xs font-medium px-5 py-3 uppercase tracking-wide">Quick Actions</th>
            </tr>
          </thead>
          <tbody>
            {overdueInvoices.map(invoice => {
              ...
              return (
                <tr key={invoice.id} className="border-b border-slate-700/50 hover:bg-slate-700/30 transition-colors">
                  <td className="px-5 py-3 text-slate-200 text-sm font-medium">{invoice.customerName}</td>
                  <td className="px-5 py-3 text-slate-200 text-sm font-semibold">${invoice.amount.toFixed(2)}</td>
                  <td className="px-5 py-3">
                    <span className={`text-sm font-medium ${urgencyColor}`}>{invoice.daysOverdue} Days</span>
                  </td>
                  <td className="px-5 py-3 text-slate-400 text-sm">{invoice.lastContacted ?? 'Never'}</td>
                  <td className="px-5 py-3">
